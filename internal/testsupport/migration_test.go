// This test lives in testsupport because it tests a goose migration
// rather than any one production package, and testsupport is what owns
// the test database's lifecycle. It reconstructs the pre-migration shape
// (no unique constraint, duplicate-URL feeds) and runs the migration's
// real Up SQL against it.
...
// migrationUpSQL returns the Up half of the feed-url-unique migration.
func migrationUpSQL(t *testing.T) string {
	t.Helper()
	matches, err := filepath.Glob("../../sql/schema/*_add_feed_url_unique.sql")
	if err != nil || len(matches) != 1 {
		t.Fatalf("locating the feed-url-unique migration: matches=%v err=%v", matches, err)
	}
...
// same feed URL: the migration must collapse the duplicates instead of
// aborting on "could not create unique index".
func TestFeedURLUniqueMigrationCollapsesDuplicates(t *testing.T) {
	db := testsupport.OpenTestDB(t)
	up := migrationUpSQL(t)

	// Roll the schema back to its pre-migration shape. If the migration
	// under test fails, put the constraint back so later runs start clean.
	mustExec(t, db, "dropping feeds_url_key", `alter table feeds drop constraint if exists feeds_url_key`)
	t.Cleanup(func() {
		if !feedURLKeyExists(t, db) {
			mustExec(t, db, "restoring feeds_url_key", up)
		}
	})
	...
	// The migration must survive this database, not abort on it.
	if _, err := db.ExecContext(context.Background(), up); err != nil {
		t.Fatalf("migration failed on a database with duplicate feed urls: %v", err)
	}
